...
public class GT_MetaTileEntity_OilDrill extends GT_MetaTileEntity_MultiBlockBase {
    ...
    public boolean checkRecipe(ItemStack aStack) {
        if (mInventory[1] == null || (mInventory[1].isItemEqual(ItemList.Block_MiningPipe.get(1)) && mInventory[1].stackSize < mInventory[1].getMaxStackSize())) {
            ArrayList<ItemStack> tItems = getStoredInputs();
            for (ItemStack tStack : tItems) {
                if (tStack.isItemEqual(ItemList.Block_MiningPipe.get(1))) {
                ...
                if (mInventory[1] == null) {
                    mInventory[1] = ItemList.Block_MiningPipe.get(1);
                } else {
    ...
    private boolean moveOneDown() {
        if ((this.mInventory[1] == null) || (this.mInventory[1].stackSize < 1)
                || (!GT_Utility.areStacksEqual(this.mInventory[1], ItemList.Block_MiningPipe.get(1)))) {
            return false;
        }
        ...
        if (!(getBaseMetaTileEntity().getWorld().setBlock(getBaseMetaTileEntity().getXCoord() + xDir, yHead - 1, getBaseMetaTileEntity().getZCoord() + zDir, GT_Utility.getBlockFromStack(ItemList.Block_MiningPipeTip.get(1))))) {
            return false;
        }
        if (yHead != getBaseMetaTileEntity().getYCoord()) {
            getBaseMetaTileEntity().getWorld().setBlock(getBaseMetaTileEntity().getXCoord() + xDir, yHead, getBaseMetaTileEntity().getZCoord() + zDir, GT_Utility.getBlockFromStack(ItemList.Block_MiningPipe.get(1)));
        }
    ...
    private int getYOfPumpHead() {
        ...
        while (getBaseMetaTileEntity().getBlock(getBaseMetaTileEntity().getXCoord() + xDir, y, getBaseMetaTileEntity().getZCoord() + zDir) == GT_Utility.getBlockFromStack(ItemList.Block_MiningPipe.get(1))) {
            y--;
        }
        if (y == getBaseMetaTileEntity().getYCoord() - 1) {
            if (getBaseMetaTileEntity().getBlock(getBaseMetaTileEntity().getXCoord() + xDir, y, getBaseMetaTileEntity().getZCoord() + zDir) != GT_Utility.getBlockFromStack(ItemList.Block_MiningPipeTip.get(1))) {
                return y + 1;
            }
                ...
                .getBlockFromStack(ItemList.Block_MiningPipeTip.get(1)) && this.mInventory[1] != null && this.mInventory[1].stackSize > 0 && GT_Utility.areStacksEqual(this.mInventory[1], ItemList.Block_MiningPipe.get(1))) {
