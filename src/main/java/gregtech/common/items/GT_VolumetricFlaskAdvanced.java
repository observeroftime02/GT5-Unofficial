...
public class GT_VolumetricFlaskAdvanced extends GT_Generic_Item implements IFluidContainerItem {
    private final int maxCapacity;
    @SideOnly(Side.CLIENT)
    public IIcon iconWindow;

    public GT_VolumetricFlaskAdvanced(String unlocalized, String english, int maxCapacity) {
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float xOffset, float yOffset, float zOffset) {
        ...
        if (mop.typeOfHit == MovingObjectType.BLOCK) {
            ...
            FluidStack fluidStack = drainContainerStack(stack, player, 128000, true);
            if (placeFluid(fluidStack, world, x, y, z) || (player.canPlayerEdit(x + dir.offsetX, y + dir.offsetY, z + dir.offsetZ, mop.sideHit, stack) && placeFluid(fluidStack, world, x + dir.offsetX, y + dir.offsetY, z + dir.offsetZ))) {
                if (!player.capabilities.isCreativeMode)
                    drainContainerStack(stack, player, 128000, false);
                return true;
            }
        }
        return false;
    ...
    public void registerIcons(IIconRegister aIconRegister) {
        ...
        iconWindow = aIconRegister.registerIcon(RES_PATH_ITEM + "gt.Volumetric_Flask_Advanced.window");
    }
